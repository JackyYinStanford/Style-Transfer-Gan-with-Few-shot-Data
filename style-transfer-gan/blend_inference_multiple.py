...
parser.add_argument('--swap', action='store_true', help='if swap, using the specified swap strategy')
parser.add_argument('--layers', type=int, nargs='+', default=[128, 256], help='specify which layers set to be used for latter models')
parser.add_argument('--dataset', type=str, default='test_face', help='specify which reconstructed npy dataset to use')
...
# prefix and name should correspond to each other
for count in counts:
    # Create a folder for each count to store visualizations and model results
    vis_path = f'./vis/{name}/{count}/'
    model_path = f'./checkpoints/{name}/{count}/'
    os.makedirs(vis_path, exist_ok=True)
    os.makedirs(model_path, exist_ok=True)
    
    # Blend and check effects for different resolutions, using res
    for res in args.layers:
        ...
        if not os.path.isfile(checkpoint):
            # Layers greater than res will use ffhq, layers less than res will use finetune results.
            # blend_width is not used here, defaulting to all model1 or all model2
        ...
        for latent_file in tqdm(latents):
            ...
            Image.fromarray(images.transpose((0, 2, 3, 1))[0], 'RGB').save(os.path.join(save_pth, f"{latent_file.stem}.jpg"))