...
class Pix2PixAttentionGanModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_L1', type=float, default=250.0, help='weight for L1 loss')
            ...
            parser.add_argument('--lambda_mask_saturate', type=float, default=0, help='weight for mask_saturate')
            parser.add_argument('--lambda_mask_smooth', type=float, default=0, help='weight for mask_smooth')
    ...
    def __init__(self, opt):
        ...
        if opt.use_vgg_loss:
            self.loss_names = ['G_GAN', 'G_L1', 'VGG_LOSS', 'D_real', 'D_fake']
        else:
            self.loss_names = ['G_GAN', 'G_L1', 'D_real', 'D_fake']
        self.perceptual_loss = PerceptualLoss()
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        self.visual_names = ['real_A', 'fake_B', 'fake_B_mask', 'real_B']
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(self.netG.parameters(), lr=opt.lr, weight_decay=0.0001, betas=(opt.beta1, 0.999))
            self.optimizers.append(self.optimizer_G)

            self.optimizer_D = torch.optim.Adam(self.netD.parameters(), lr=opt.lr, weight_decay=0.0001, betas=(opt.beta1, 0.999))
    ...
    def backward_G(self):
        ...
        if self.opt.use_vgg_loss:
            ...
            # self.loss_G = self.loss_G_L1 + self.loss_Saturate + self.loss_Smooth + self.loss_VGG_LOSS + self.loss_G_GAN
            self.loss_G = self.loss_G_L1 + self.loss_VGG_LOSS + self.loss_G_GAN
        else:
            # self.loss_G = self.loss_G_L1 + self.loss_Saturate + self.loss_Smooth + self.loss_G_GAN
