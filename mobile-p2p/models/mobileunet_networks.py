...
#             if use_v1:
#                 if use_cbam:
#                     attention_layer = CBAM(inp, reduction_ratio=self.reduction_ratio)
#                 else:
#                     attention_layer = SEModule(inp, reduction_ratio=self.reduction_ratio, use_fc=self.use_fc)
#             else:
#                 if use_cbam:
#                     attention_layer = CBAM(hidden_dim, reduction_ratio=self.reduction_ratio)
#                 else:
#                     attention_layer = SEModule(hidden_dim, reduction_ratio=self.reduction_ratio, use_fc=self.use_fc)

#         if use_v1:
#             if self.reduction_ratio > 0:
#                 model = [
#                     nn.Conv2d(inp, inp, 3, stride, 1, groups=inp, bias=use_bias),
#                     norm_layer(inp),
#                     # SEModule(inp, reduction_ratio=self.reduction_ratio, use_fc=self.use_fc),
#                     attention_layer,
#                     nn.ReLU(inplace=True),
...
#                     norm_layer(hidden_dim),
#                     #SEModule(hidden_dim, reduction_ratio=self.reduction_ratio, use_fc=self.use_fc),
#                     attention_layer,
#                     nn.ReLU(inplace=True),
#                     # pw-linear
...
#                          nn.Conv2d(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=use_bias),
#                          norm_layer(hidden_dim),
#                          #SEModule(hidden_dim, reduction_ratio=self.reduction_ratio, use_fc=use_fc),
#                          attention_layer,
#                          nn.ReLU(inplace=True),
#                          nn.Conv2d(hidden_dim, oup, 1, 1, 0, bias=use_bias)]
#             else:
...
class MobileUnet(nn.Module):
    def __init__(self, layer_size=6, channel=8, expand=2, gpu=1, norm_layer=nn.BatchNorm2d, use_residual=True,
                 with_mask=False, input_h=0, input_w=0, reduction_ratio=0, use_fc=0, use_cbam=0, with_warp = False, with_hair=False):
...
#             #print('with_hair!!!!!!!\n\n')
#             h = self.tanh(h)
#             h, mask, hair_rgba = h[:,:3,:,:], h[:,3:4,:,:], h[:,4:,:,:]
#             self.rgb = h
#             self.alpha = mask
#             self.hair_rgba = hair_rgba
#             return h,mask,hair_rgba

        if self.with_hair:
            h_face = self.tanh(h_face)
            h_hair = self.dec_0_hair(h)
...
class MobileUnetTNN(nn.Module):
    def __init__(self, layer_size=6, channel=8, expand=2, gpu=1, norm_layer=nn.BatchNorm2d, use_residual=True,
                 with_mask=False, with_hair=False, input_h=0, input_w=0, reduction_ratio=0, use_fc=0, use_cbam=0):
        ...
        self.with_hair = with_hair
        if with_hair:
            self.dec_0 = UpResidual(64 // channel * 1, 8, 1, expand_ratio=expand, bn=False, norm_layer=norm_layer,
                                out_h=self.input_h, out_w=self.input_w,
                                reduction_ratio=reduction_ratio, use_fc=use_fc, use_v1=False, use_cbam=use_cbam)
        elif with_mask:            
            self.dec_0 = UpResidual(64 // channel * 1, 4, 1, expand_ratio=expand, bn=False, norm_layer=norm_layer,
